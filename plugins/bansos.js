  ...
  try {
    ...
    if (new Date - global.db.data.users[m.sender].lastbansos > 300000) {
      if (Aku > Kamu) {
        ...
        user.money -= 9000000000000
        global.db.data.users[m.sender].lastbansos = new Date * 1
      } else if (Aku < Kamu) {
        user.money += 90000000000000
        conn.sendFile( m.chat, mbansos, 'b.jpg', `Kamu berhasil  korupsi dana bansos🕴️💰,  Dan kamu mendapatkan 90 Juta rupiah💵`, m)
        global.db.data.users[m.sender].lastbansos = new Date * 1
      } else {
